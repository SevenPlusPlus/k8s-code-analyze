...
最后了解一下type DeltaFIFO struct的实现。Reflector机制的store是一个type DeltaFIFO struct对象，Reflector保证只会把符合expectedType类型的对象存放到store中。
...
5. 各endpointcontroller的podInformer注册调用AddEventHandler会在sharedIndexInformer中生成type processorListener struct。

6. type Controller struct会把消息分发到各个listener的addCh channel中，listener类型是type processorListener struct。
...
### 参考资料
[ControllerManager的List-Watch机制-1](https://github.com/Kevin-fqh/learning-k8s-source-code/blob/master/controller-manager/\(02\)ControllerManager%E7%9A%84List-Watch%E6%9C%BA%E5%88%B6-2.md)